...
export default function SkillsManager() {
  const { data, addSkill, deleteSkill, updateSkill, addToast } = useData();
  const [isAdding, setIsAdding] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [newSkill, setNewSkill] = useState(initialSkill);
  const [activeCategory, setActiveCategory] = useState('all');
  const [confirmDelete, setConfirmDelete] = useState({ isOpen: false, skillId: null, skillName: '', category: '' });
  ...
  const handleAdd = () => {
    ...
    addToast('success', 'Skill added successfully', `"${newSkill.name}" has been added to ${categoryLabels[newSkill.category]}.`);
  ...
  const handleUpdate = () => {
    ...
    addToast('success', 'Skill updated successfully', `"${newSkill.name}" has been updated.`);
    setEditingId(null);
    setNewSkill(initialSkill);
    setIsAdding(false);
  };
  ...
  const openDeleteConfirm = (skill, category) => {
    setConfirmDelete({ isOpen: true, skillId: skill.id, skillName: skill.name, category });
  };

  const closeDeleteConfirm = () => {
    setConfirmDelete({ isOpen: false, skillId: null, skillName: '', category: '' });
  };

  const handleDelete = () => {
    deleteSkill(confirmDelete.skillId, confirmDelete.category);
    addToast('success', 'Skill deleted', `"${confirmDelete.skillName}" has been removed.`);
    closeDeleteConfirm();
  };
  ...
  return (
    <>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        onClick={() => openDeleteConfirm(skill, skill.category)}
      ...
      <ConfirmDialog
        ...
        itemName={confirmDelete.skillName}
      />
    </>
  );
}
